...
def menu(collection, *widgets):
    ...
    if isinstance(collection, str):
        globals.vol = globals.db[collection]
        string_collection = collection
    else:
        globals.vol = collection

    db_label = Label(globals.root, text="Collection Name: " + string_collection, font="Helvetica 18 bold")
    db_label.place(x=globals.root.winfo_width()/3 - 10, y=0)
